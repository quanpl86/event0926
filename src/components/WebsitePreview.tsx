...
import { fillVoice, isPrimary } from "@/lib/age";

export function WebsitePreview({ name, projectName, gradeBand, quote }: { name: string; projectName: string; gradeBand: string; quote: string }) {
  const projects = [
    { icon: Bot, name: projectName || (isPrimary(gradeBand) ? "Robot bạn thân" : "Smart Robot") },
    { icon: Gamepad2, name: isPrimary(gradeBand) ? "Game nhỏ" : "Mini Game" },
    { icon: Globe2, name: isPrimary(gradeBand) ? "Thế giới vui" : "Digital World" }
  ];
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-soft">
      ...
      <div className="bg-tek-50 px-5 py-8 text-center">
        ...
        <p className="text-[10px] font-extrabold tracking-[.18em] text-tek-600">{isPrimary(gradeBand) ? "YOUNG CREATOR" : "STUDENT CREATOR"}</p>
        <h3 className="mt-2 text-2xl font-extrabold tracking-tight text-ink">Xin chào, mình là {name || "Minh"}!</h3>
        <p className="mt-2 text-sm text-slate-600">“{quote}”</p>
      </div>
      ...
      <div className="flex items-center gap-2 border-t border-slate-100 px-5 py-3 text-[10px] font-bold text-slate-500"><Sparkles className="h-4 w-4 text-amber-400" /> {fillVoice(`Website sẽ lớn lên cùng mỗi dự án của {you}.`, gradeBand)}</div>
    </div>
  );
}
